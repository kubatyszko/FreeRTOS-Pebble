...
def load_resource_from_pbpack(fname, resid):
    ...
    with open(fname, 'r') as fd:
        nrsrc = struct.unpack('I', fd.read(4))[0]
        
        for i in range(nrsrc):
            fd.seek(TAB_OFS + i * 16)
            (idx, ofs, sz, crc) = struct.unpack('iiiI', fd.read(16))
            
            if idx == resid:
                break
        
        if idx != resid:
            raise IOError("res %d not found in pbpack \"%s\"" % (resid, fname, ))
...
class ResourceCollection:
    ...
    class ResourceRef(Resource):
        ...
        def sourcedesc(self):
            return "resource ID %d from pack %s" % (self.resid, self.resfile)
    
    class ResourceFile(Resource):
        def __init__(self, coll, j):
            super(self.__class__, self).__init__(coll, j)
            
            self.file = "%s/%s" % (self.coll.root, j["input"]["file"], )
    ...
    def __init__(self, fname, root = "."):
        ...
        if "references" in jdb:
            self.references = {k: "%s/%s" % (self.root, v) for k, v in jdb["references"].iteritems()}
        else:
            self.references = {}
        ...
        for res in jdb["resources"]:
            if res["input"]["type"] == "file":
                self.resources.append(ResourceCollection.ResourceFile(self, res))
            elif res["input"]["type"] == "resource":
                self.resources.append(ResourceCollection.ResourceRef(self, res))
            else:
                raise ValueError("unknown resource type %s" % (res["type"], ))
    ...
    def write_header(self, fname):
        ...
        with open(fname, 'w') as fd:
            fd.write("/* THIS FILE IS AUTOMATICALLY GENERATED BY mkpack.py. */\n")
            fd.write("/* IF YOU MODIFY IT, YOU WILL BE VERY SAD. */\n")
            fd.write("/* IF YOU CHECK IT IN, I WILL BE VERY SAD. */\n")
            fd.write("\n")
            fd.write("#pragma once\n")
            fd.write("\n")
            fd.write("typedef enum resource_id {\n")
            for (id, r) in enumerate(self.resources):
                fd.write("    %s = %d, /* (from %s) */\n" % (r.name, id + 1, r.sourcedesc(), ))
            fd.write("} resource_id;\n")
    ...
    def write_makedeps(self, fname, rsrcfile, hdrfile):
        ...
        with open(fname, 'w') as fd:
            fd.write("# automatically generated by mkpack.py\n\n")
            fd.write("%s: %s %s " % (rsrcfile, self.jfname, __file__, ))
            for d in self.deps():
                fd.write("%s " % (d, ))
            fd.write("\n\n")
            fd.write("%s: %s\n\n" % (hdrfile, rsrcfile, ))
            for d in self.deps():
                fd.write("%s:\n\n" % (d, ))
            fd.write("# That will conclude this evening's entertainment.\n")
...
def main():
    ...
    rc = ResourceCollection(args.json, root = args.root)
    
    pbpack_name = "%s.pbpack" % (args.basename, )
    header_name = "%s.h" % (args.basename, )
    dep_name = "%s.d" % (args.basename, )
    if args.pbpack:
        bytes = rc.write_pbpack(pbpack_name)
        print("wrote %s (%d bytes)" % (pbpack_name, bytes, ))
    if args.header:
        rc.write_header(header_name)
        print("wrote %s" % (header_name, ))
    if args.make_dep:
        rc.write_makedeps(dep_name, pbpack_name, header_name)
        print("wrote %s" % (dep_name, ))
